...
library(Hmisc)
library(plyr)
# library(corrplot) funktioniert nicht so schön wie gedacht
...
kable(VIF, align='c', digits=2, caption='Variance Inflation Factor')
```
...
Nevertheless, we went ahead and created two models using ordinal logistic regression, whose output is shown in the table below.
...
Since $\beta$'s in logistic regression are a bit weird, we need to transform these results into predicted probabilities in your final project.
