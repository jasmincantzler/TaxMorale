...
**Research Question**: What are the determinants of tax morale in Africa?. 

**Data Sources**: The data we will use data to investigate this question is provided by the [Afrobarometer](http://www.afrobarometer.org/) and freely available for the years 2015, 2008, and 2005.

The data is downloaded directly from the Afrobarometer's website into R using the following code:

```{r}
install.packages("rio") ## needed to read data as it is stored in SPSS format
library(rio)
main <- rio::import("http://afrobarometer.org/sites/default/files/data/round-5/merged_r5_data_0.sav")
```

The data sets are available individually for the single years so some merging and cleaning of the data will be necessary before conducting the analysis. 
